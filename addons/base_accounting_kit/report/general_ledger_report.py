...
class ReportGeneralLedger(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby,
        ...
        if init_balance:
            ...
            filters = init_filters.replace('account_move_line__move_id',
                                           'm').replace('account_move_line',
                                                        'l')
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, NULL AS move_id, ''
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, m.id AS move_id,
        ...
        COALESCE(SUM(l.credit), 0) AS balance,\
            m.name AS move_name, c.name AS currency_code, p.name AS
            partner_name\
            FROM account_move_line l\
            ...
            l.account_id, m.id, l.date, j.code, l.currency_id, l.amount_currency,
            l.ref, l.name, m.name, c.name, p.name ORDER BY ''' + sql_sort)
