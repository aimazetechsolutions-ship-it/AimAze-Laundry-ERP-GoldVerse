...
function getGoldverseSearchItemByName(searchModel, name) {
    return searchModel?.getSearchItems((item) => item.name === name)?.find((item) => item);
}

function getGoldverseCustomFilterItems(searchModel) {
    return (
...
function getGoldverseActiveQueryEntries(searchModel, item) {
...
patch(SearchBar.prototype, {
    ...
    get isGoldverseSearchToolbar() {
        ...
        const hasPeriodMarkers = ["today", "gv_mtd", "gv_ytd", "gv_itd", "gv_custom"].some(
            (name) => Boolean(getGoldverseSearchItemByName(this.env.searchModel, name))
        );
    ...
    get hasGoldverseSearchPeriodFilters() {
        return Object.values(PERIOD_FILTER_NAMES).every((name) =>
            Boolean(getGoldverseSearchItemByName(this.env.searchModel, name))
        );
    },
    ...
    goldverseSyncSearchToolbarState() {
        ...
        let activePeriod = "today";
        for (const [period, searchName] of Object.entries(PERIOD_FILTER_NAMES)) {
            const item = getGoldverseSearchItemByName(this.env.searchModel, searchName);
            if (item && getGoldverseActiveQueryEntries(this.env.searchModel, item).length) {
                activePeriod = period;
                break;
            }
        }

        const customItem = activePeriod === "today" && getGoldverseCustomFilterItems(this.env.searchModel).find(
            (item) => getGoldverseActiveQueryEntries(this.env.searchModel, item).length
        );
        if (customItem) {
            activePeriod = "custom";
            const marker = String(customItem.description || "").replace(CUSTOM_FILTER_PREFIX, "");
    ...
    goldverseClearSearchDateFilters() {
        ...
        const customDateItem = getGoldverseSearchItemByName(this.env.searchModel, "gv_custom");
        if (customDateItem) {
            for (const entry of getGoldverseActiveQueryEntries(this.env.searchModel, customDateItem)) {
                this.env.searchModel.toggleDateFilter(customDateItem.id, entry.generatorId);
            }
        }

        for (const item of getGoldverseCustomFilterItems(this.env.searchModel)) {
            if (getGoldverseActiveQueryEntries(this.env.searchModel, item).length) {
                this.env.searchModel.deactivateGroup(item.groupId);
            }
        }
    },
    ...
    goldverseApplySearchPeriod(period) {
        ...
        const item = getGoldverseSearchItemByName(this.env.searchModel, PERIOD_FILTER_NAMES[period]);
        if (item) {
            this.env.searchModel.toggleSearchItem(item.id);
        }
    ...
    goldverseApplySearchCustomerType(customerType) {
        ...
        const item = getGoldverseSearchItemByName(this.env.searchModel, CUSTOMER_FILTER_NAMES[customerType]);
        if (item) {
            this.env.searchModel.toggleSearchItem(item.id);
        }
        this.goldverseSearchToolbarState.customerType = customerType;
